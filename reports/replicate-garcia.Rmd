...
- Restricted to
  ...
  - No job history pattern with 1977
  - Record has reliable job history data (not an odd end)
  - Eligible for cancer follow-up (2004, 2009, and 2015)
- Leaving work and cancer incidence
	- FU spans 1985--1994
- MWF exposure and leaving work
	- FU spans 3 years after hire throuh 1994, death, or leaving work
...
cancer_male.tab1 <- get.tab1(copy(cohort2[studyno %in% cohort2_male[canc_first == 1 & (ddiag_first <= yoc | is.na(yoc)),studyno]]), mathmode = F)
cancer_pr.tab1 <- get.tab1(copy(cohort2[studyno %in% cohort2_male[canc_pr == 1 & (ddiag_pr <= yoc | is.na(yoc)),studyno]]), mathmode = F)
cancer_lu.tab1 <- get.tab1(copy(cohort2[studyno %in% cohort2_male[canc_lu == 1 & (ddiag_lu <= yoc | is.na(yoc)),studyno]]), mathmode = F)
cancer_corec.tab1 <- get.tab1(copy(cohort2[studyno %in% cohort2_male[canc_corec == 1 & (ddiag_corec <= yoc | is.na(yoc)),studyno]]), mathmode = F)
cancer_female.tab1 <- get.tab1(copy(cohort2[studyno %in% cohort2_female[canc_first == 1 & (ddiag_first <= yoc | is.na(yoc)),studyno]]), mathmode = F)
hwse3.tab1 <- get.tab1(copy(cohort2_big[year <= year(jobloss.date)]), mathmode = F)
...
- If cancer incidence date was the same as the date of leaving work, it was considered to have occurred after leaving work
