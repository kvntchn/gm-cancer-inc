...
hwse2.exposure.lag <- 21
hwse3.additional.lag <- 0
hwse2.additional.lag <- 20
spline_yin <- T
spline_year <- T
time_scale <- "age"
...
tab1 <- get.tab1(
	df = as.data.table(as.data.frame(cohort_analytic[year >= 1973 & immortal == 0 & right.censored == 0])),
...
tab1 <- readRDS(
	here::here("resources", paste0("tab1", "_lag", exposure.lag, '.rds')))
...
\label{fig:hwse}
\centering
\includegraphics{"`r here::here(paste0("reports/resources/hwse 2/Lag ", 1 + hwse2.additional.lag, ifelse(employment_status.lag == 0, "", paste0("/Employment status lagged ", employment_status.lag, " years"))), paste0("hwse", paste0("_sol", messy_sol %/% .01), "_facet.pdf"))`"}
\end{figure}
...
hwse2.directory <- gsub("//", "/", here::here(paste(
	"./resources/",
	paste0("hwse 2/Lag ", hwse2.exposure.lag,
